...
if uploaded_file is not None:
    ...
    if df is not None:
        ...
        df_filtered = df[
            ...
            (df['Velocity'] >= speed_range[0]) & 
            (df['Velocity'] <= speed_range[1])
        ].copy()
        ...
        with col1:
            st.metric("Distância total", f"{df_filtered['Odometer'].max():.0f} m")
        with col2:
            st.metric("Velocidade máxima", f"{df_filtered['Velocity'].max():.1f} km/h")
        with col3:
            st.metric("Velocidade média", f"{df_filtered['Velocity'].mean():.1f} km/h")
        with col4:
            st.metric("FC média", f"{df_filtered['HeartRate'].mean():.0f} bpm")
        with col5:
            st.metric("FC máxima", f"{df_filtered['HeartRate'].max():.0f} bpm")
        ...
        with tab1:
            ...
            fig_map.add_trace(go.Scattermapbox(
                ...
                marker=dict(
                    size=6,
                    color=df_filtered['Velocity'],
                    colorscale='Viridis',
                    showscale=True,
                    colorbar=dict(title="Velocidade<br>(km/h)", x=1.02),
                    cmin=df_filtered['Velocity'].min(),
                    cmax=df_filtered['Velocity'].max()
                ),
                text=[f"<b>Tempo:</b> {t:.1f}s<br><b>Velocidade:</b> {v:.1f} km/h<br><b>FC:</b> {h:.0f} bpm" 
                      for t, v, h in zip(df_filtered['Seconds'], df_filtered['Velocity'], df_filtered['HeartRate'])],
                hoverinfo='text',
                name='Percurso'
            ))
        ...
        with tab2:
            ...
            fig.add_trace(
                go.Scatter(x=df_filtered['Velocity'], y=df_filtered['HeartRate'],
                          ...
                          marker=dict(size=5, color=df_filtered['Seconds'], 
        ...
        st.markdown(f"**📊 Resumo da análise:** {len(df_filtered)} registros | "
                   f"Período: {time_range[0]:.0f}s - {time_range[1]:.0f}s | "
                   f"Filtro de velocidade: {speed_range[0]:.1f} - {speed_range[1]:.1f} km/h")
